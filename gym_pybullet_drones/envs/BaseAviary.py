...
class BaseAviary(gym.Env):
    ...
    def _addObstacles(self):
        ...
        table_1 = p.loadURDF("table/table.urdf",
                [0, 0, 0],
                #    p.getQuaternionFromEuler([0,0,0]),
                physicsClientId=self.CLIENT
                )
        self.static_obstacles.append(table_1)


        table_2 = p.loadURDF("table/table.urdf",
                [1, -1.5, 0],
                #    p.getQuaternionFromEuler([0,0,0]),
                physicsClientId=self.CLIENT
                )
        self.static_obstacles.append(table_2)

        if self.single_agent:
                      
            table_3 = p.loadURDF("table/table.urdf",
                    [0, 0, 0.6],
                    #    p.getQuaternionFromEuler([0,0,0]),
                    physicsClientId=self.CLIENT
                    )
            self.static_obstacles.append(table_3)

            # table_4 = p.loadURDF("table/table.urdf",
            #         [1, -1.5, 0.6],
            #         #    p.getQuaternionFromEuler([0,0,0]),
            #         physicsClientId=self.CLIENT
            #         )
            # self.static_obstacles.append(table_4)
