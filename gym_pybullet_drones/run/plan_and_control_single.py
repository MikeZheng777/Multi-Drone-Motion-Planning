...
def run(
    ...
    H_STEP = 0.1
    R = .4
    INIT_XYZS = np.array([[R*np.cos((i/6)*2*np.pi+np.pi/2), R*np.sin((i/6)*2*np.pi+np.pi/2)-R, H+i*H_STEP] + np.array([2,-2,0]) for i in range(num_drones)])
    INIT_RPYS = np.array([[0, 0,  i * (np.pi/2)/num_drones] for i in range(num_drones)])

    GOAL_XYZS = np.array([[R*np.cos((i/6)*2*np.pi+np.pi/2), R*np.sin((i/6)*2*np.pi+np.pi/2)-R, H+i*H_STEP] + np.array([0.1,0.1,1]) for i in range(num_drones)])
    ...
    if reload:
        drone_trajs = np.load('results/drone_trajs_single.npy')
        ...
        for i in range(num_drones):
           planner = BiRRT_Star(INIT_XYZS[i], GOAL_XYZS[i], static_obstacles=env.static_obstacles, dynamic_obstacles=[], bounds=((0, 1), (0, 1), (0, 1)), goal_bias=0.1)
           traj = planner.get_traj()
           drone_trajs.append(traj)
        
        drone_trajs_smooth = np.array([smooth_trajectory(traj) for traj in drone_trajs])
        np.save('results/drone_trajs_single.npy',drone_trajs)
        
    if debug:
        step = 10
        for traj in drone_trajs_smooth:
        #     for pos in traj:
        #         p.loadURDF("sphere_small.urdf", pos)
    ...
    for i in range(1,len(drone_trajs_smooth[0])):
        ...
        for j in range(num_drones):
            logger.log(drone=j,
                    ...
                    #    control=np.hstack([TARGET_POS[wp_counters[j], 0:2], INIT_XYZS[j, 2], INIT_RPYS[j, :], np.zeros(6)])
                       control=np.hstack([drone_trajs_smooth[j][i], INIT_RPYS[j, :], np.zeros(6)])
    ...
    # logger.save()
    # logger.save_as_csv("single") # Optional CSV save
