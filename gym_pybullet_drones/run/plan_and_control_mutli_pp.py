...
from gym_pybullet_drones.planner.prioritized_planning import *
...
def run(
    ...
    # import pdb; pdb.set_trace()
    if reload:
        drone_trajs_smooth = np.load('results/drone_trajs_multi.npy')
        # drone_trajs_smooth = np.array([smooth_trajectory(traj, num_samples=500) for traj in drone_trajs_smooth])
        # for i, traj in enumerate(drone_trajs):
    else:
        # drone_trajs = []
        planner = Prioritized_Planing(env = env, order=[0,1,2], inital_xyz=INIT_XYZS, final_xyz=GOAL_XYZS)
        drone_trajs = planner.get_all_traj()
    ...
    for i in range(len(drone_trajs_smooth[2])):
    ...
    # logger.save()
    # logger.save_as_csv("multi_pp") # Optional CSV save
